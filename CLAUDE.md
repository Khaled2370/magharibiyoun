...
## ⚠️ Upload d'images cassé en production (constaté le 2026-09-05, action côté Khaled)
Un envoi d'image en production renvoie `?uploadError=config` : les variables `CLOUDINARY_CLOUD_NAME` / `CLOUDINARY_API_KEY` / `CLOUDINARY_API_SECRET` ne sont **pas rattachées au projet** sur Vercel (elles avaient été ajoutées en « Shared » en juillet, jamais vérifiées depuis). Le site fonctionne, seules les images ne s'enregistrent pas ; le message l'indique explicitement depuis `CloudinaryNotConfiguredError`. Remède : ajouter les trois variables **dans le projet lui-même** (Environment Variables → Production) puis redéployer.
