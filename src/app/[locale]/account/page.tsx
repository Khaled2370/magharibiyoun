...
export default async function AccountPage({
  ...
  // `requireUser` renvoie une session dont l'identifiant est vérifié, ou
  // redirige vers la connexion. Jamais un simple « l'objet session existe ».
  ...
  // Le cookie désigne un compte qui n'existe plus (compte supprimé, base
  // réinitialisée). Surtout NE PAS rediriger vers la page de connexion : celle-ci
  // voit une session et renverrait ici — boucle infinie, personne ne peut plus
  // ni se connecter ni se déconnecter. On affiche donc une sortie explicite.
  if (!user) {
    return (
      <div className="mx-auto max-w-md px-4 py-16 text-center">
        <h1 className="text-2xl font-medium">{ta("staleSessionTitle")}</h1>
        <p className="mt-2 text-sm text-mutedink">{ta("staleSessionText")}</p>
        <form
          className="mt-6"
          action={async () => {
            "use server";
        ...
        >
          <button
            type="submit"
            className="rounded-lg bg-majorelle px-5 py-2.5 text-sm font-medium text-white transition-opacity hover:opacity-90"
          >
            {ta("staleSessionCta")}
          </button>
        </form>
